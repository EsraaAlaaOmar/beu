...
const Box = ({clearstate,product,editeLink='/dashbord/collections',viewProducts, products=true, deleteFuction, offer_id,brandId}) => {
  const dispatch = useDispatch()
  const  productColors =product.gallerie && product.galleries.length >0 && product.galleries.map(galary=>{
  ...
  return (
    <div className='box_component'>
        <img
         src={product.gallerie &&product.galleries.length&&product.galleries[0].image}
          />
          ...
          <div className='yello'>
              ...
              <div>Sizes :{product.size&&product.size.map(s=><span className='size'>{s.size}</span>)}</div>
